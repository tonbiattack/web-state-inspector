...
`State Change Timeline`、`Errors`、`Recordings`、`Compare`、`Snapshots`、IndexedDB、Cache Storage の独立画面は廃止しました。Storage 変更とエラーは Timeline に統合しています。
...
パネル表示中の **F5** は、検査対象ページを再読み込みします。入力欄や再現メモの編集中はショートカットを奪いません。
...
`pnpm run verify` は型検査、拡張の組み立て、回帰テストを実行します。テストには Timeline 集約、任意イベントの Context Window、Network 詳細の開閉保持、`Pause updates` / `Resume updates`、AI Export を含みます。

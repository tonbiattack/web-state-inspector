import type { ChangeTrackerStatus, ChangeTrackingSnapshot, StorageChangeEvent } from '../shared/types.js';
...
export class ChangeTracker {
  constructor(private readonly evaluator: PageEvaluator) {}

  async start(capacity = DEFAULT_CAPACITY): Promise<{ ok: boolean; data?: ChangeTrackerStatus; error?: string }> {
    const safeCapacity = Math.min(Math.max(capacity, 20), 1000);
    return this.evaluator.evaluate<ChangeTrackerStatus>(`(() => {
  ...
  getSnapshot(): Promise<{ ok: boolean; data?: ChangeTrackingSnapshot; error?: string }> {
    return this.evaluator.evaluate<ChangeTrackingSnapshot>(`(() => {
  ...
  stop(): Promise<{ ok: boolean; data?: ChangeTrackerStatus; error?: string }> {
    return this.evaluator.evaluate<ChangeTrackerStatus>(`(() => {
      const state = window[Symbol.for(${JSON.stringify(TRACKER_SYMBOL)})];
  ...
  clear(): Promise<{ ok: boolean; data?: ChangeTrackerStatus; error?: string }> {
    return this.evaluator.evaluate<ChangeTrackerStatus>(`(() => {
      const state = window[Symbol.for(${JSON.stringify(TRACKER_SYMBOL)})];
      if (!state) return { active: false, capacity: ${DEFAULT_CAPACITY}, eventCount: 0 };
      state.events = [];

import type { InteractionTrackingSnapshot, RouteChangeEvent, UserActionEvent } from '../shared/types.js';
...
export class InteractionTracker {
  ...
  start(): Promise<{ ok: boolean; data?: InteractionTrackingSnapshot; error?: string }> {
  ...
  getSnapshot(): Promise<{ ok: boolean; data?: InteractionTrackingSnapshot; error?: string }> {
    return this.evaluator.evaluate(`(() => {
  ...
  clear(): Promise<{ ok: boolean; data?: InteractionTrackingSnapshot; error?: string }> {
    return this.evaluator.evaluate(`(() => {
      const state = window[Symbol.for(${JSON.stringify(INTERACTION_TRACKER_SYMBOL)})];
  ...
  stop(): Promise<{ ok: boolean; data?: InteractionTrackingSnapshot; error?: string }> {
    return this.evaluator.evaluate(`(() => {
      const state = window[Symbol.for(${JSON.stringify(INTERACTION_TRACKER_SYMBOL)})];
      if (!state) return { active: false, actionCount: 0, routeCount: 0, actions: [], routes: [] };
      if (state.active) {

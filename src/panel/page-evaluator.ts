...
export class PageEvaluator {
  async evaluate<T>(expression: string): Promise<InspectResult<T>> {
   ...
   */
  async evaluateAsync<T>(operation: string): Promise<InspectResult<T>> {
    const taskId = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const started = await this.evaluate<boolean>(`(() => {
    ...
    for (let attempt = 0; attempt < POLL_ATTEMPTS; attempt += 1) {
      ...
      const polled = await this.evaluate<AsyncOperation<T> | null>(pollExpression);
      if (!polled.ok) return { ok: false, error: polled.error };
      if (!polled.data || polled.data.status === 'pending') continue;
      await this.evaluate(cleanupExpression);
      if (polled.data.status === 'error') return { ok: false, error: polled.data.error ?? '非同期処理に失敗しました。' };
  ...
  getStorage(kind: 'localStorage' | 'sessionStorage'): Promise<InspectResult<StorageEntry[]>> {
    return this.evaluate<StorageEntry[]>(`(() => {
  ...
  getIndexedDatabases(): Promise<InspectResult<IndexedDbSummary[]>> {
    return this.evaluateAsync<IndexedDbSummary[]>(`(async () => {
  ...
  getIndexedDbRecords(databaseName: string, storeName: string, limit = 100): Promise<InspectResult<IndexedDbRecord[]>> {
    const args = JSON.stringify({ databaseName, storeName, limit: Math.min(Math.max(limit, 1), 100) });
    return this.evaluateAsync<IndexedDbRecord[]>(`(async () => {
      const args = ${args};
  ...
  getCacheNames(): Promise<InspectResult<string[]>> {
    return this.evaluateAsync<string[]>(`(async () => (await caches.keys()).sort())()`);
  }

  getCacheEntries(cacheName: string, limit = 100): Promise<InspectResult<CacheSummary>> {
    const args = JSON.stringify({ cacheName, limit: Math.min(Math.max(limit, 1), 100) });
    return this.evaluateAsync<CacheSummary>(`(async () => {
      const args = ${args};
  ...
  getFrameworkState(kind: 'pinia' | 'tanstackQuery'): Promise<InspectResult<FrameworkState>> {
    const accessor = kind === 'pinia' ? 'getPinia' : 'getTanStackQuery';
    const label = kind === 'pinia' ? 'Pinia' : 'TanStack Query';
    return this.evaluateAsync<FrameworkState>(`(async () => {

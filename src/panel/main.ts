...
chrome.runtime.onMessage.addListener((message: unknown, sender, sendResponse: (response: BridgeResponse) => void) => {
  ...
  if (envelope?.type !== 'WEB_STATE_INSPECTOR_BRIDGE_REQUEST' || !envelope.request || sender.tab?.id !== chrome.devtools.inspectedWindow.tabId) return;
...
type InspectStorage = { local: StorageEntry[]; session: StorageEntry[] };
type FrameworkStates = { pinia: FrameworkState; tanstackQuery: FrameworkState };
...
const navItems: Array<{ id: NavigationId; label: string; group: 'Inspect' | 'Experimental' | 'Debug'; experimental?: boolean }> = [
...
function renderStorage(entries: StorageEntry[], expansionPrefix: string = state.selected): HTMLElement {
...
function renderInspectStorage(data: InspectStorage): HTMLElement {
  const section = element('section');
  ...
  for (const [label, entries] of [['Local Storage', data.local], ['Session Storage', data.session]] as const) {
    const details = element('details') as HTMLDetailsElement;
    details.open = true;
...
function renderShell(): void {
  ...
  for (const group of ['Debug', 'Inspect', 'Experimental'] as const) {
    ...
    navItems.filter((item) => item.group === group).forEach((item) => {
      ...
      button.addEventListener('click', () => {
        ...
        if (['debug-timeline', 'network', 'ai-export'].includes(state.selected) && !['debug-timeline', 'network', 'ai-export'].includes(item.id)) stopDebugPolling();
        state.selected = item.id;
        syncStoragePolling();
        if (['debug-timeline', 'network', 'ai-export'].includes(item.id) && debugSession.getStatus().active) startDebugPolling();
        state.query = '';
        refreshPanel();
      });
...
function setBody(content: HTMLElement): void {
  ...
  clear(body);
  body.append(content);
  if (scrollHost && !timelineFocusEventId) scrollHost.scrollTop = scrollTop;
}
...
function renderFrameworkStates(states: FrameworkStates): HTMLElement {
  const section = element('section');
  section.append(element('div', 'notice warning', 'Experimental: 明示的な読み取り専用diagnostic bridgeがある場合だけ表示します。'));
  for (const [label, result] of [['Pinia', states.pinia], ['TanStack Query', states.tanstackQuery]] as const) {
    const details = element('details') as HTMLDetailsElement;
    details.append(element('summary', undefined, `${label} · ${result.detected ? 'Detected' : 'Not detected'}`), element('p', 'summary', result.message));
    if (result.detected) details.append(jsonView(result.data, true, `framework:${label}`));
    section.append(details);
  }
  return section;
}
...
function currentFocusedEvent(timeline = debugSession.getTimeline()): TimelineEvent | undefined {
...
function focusExportOnEvent(eventId: string): void {
  focusedEventId = eventId;
  state.selected = 'ai-export';
  state.query = '';
  syncStoragePolling();
  if (debugSession.getStatus().active) startDebugPolling();
  void refreshPanel();
}
...
async function saveRecording(name: string): Promise<void> {
  ...
  const recording: DebugRecording = {
    ...
    snapshots: [beforeSnapshot, afterSnapshot].filter((snapshot): snapshot is DebugSnapshot => Boolean(snapshot)), selectedElements: selectedElementSnapshots.map((snapshot) => ({ ...snapshot, summary: { ...snapshot.summary } })),
  };
  recordings.push(recording);
  if (recordings.length > 2) recordings.splice(0, recordings.length - 2);
...
function renderRecordings(): HTMLElement {
  const section = element('section');
  section.append(element('p', 'summary', '停止したDebug Recordingをローカルに最大2件保持します。正常系と異常系を保存してCompareへ進みます。新しい記録を開始する前に保存してください。'));
  const form = element('div', 'timeline-controls'); const name = element('input', 'compact-input') as HTMLInputElement;
  name.placeholder = 'Recording name (Normal / Broken)'; name.maxLength = 80;
  const save = element('button', 'action-button', 'Save current recording'); save.type = 'button'; save.disabled = debugSession.getStatus().active || debugSession.getStatus().eventCount === 0;
  save.addEventListener('click', () => { void saveRecording(name.value); }); form.append(name, save); section.append(form);
  if (!recordings.length) { section.append(element('div', 'empty', '保存済みRecordingはありません。')); return section; }
  const { table, body } = createTable(['Name', 'Created', 'Events', 'Network', 'Errors']);
  for (const item of recordings) { const row = element('tr'); row.append(element('td', 'key-cell', item.name), element('td', undefined, formatTime(item.createdAt)), element('td', undefined, String(item.timeline.length)), element('td', undefined, String(item.network.length)), element('td', undefined, String(item.errors.length))); body.append(row); }
  section.append(table); return section;
}

function recordingSelect(value: string | undefined, label: string, onChange: (id: string | undefined) => void): HTMLElement {
  const field = element('label', 'field-label', label); const select = element('select', 'interval-select') as HTMLSelectElement;
  for (const recording of recordings) { const option = element('option') as HTMLOptionElement; option.value = recording.id; option.textContent = recording.name; option.selected = recording.id === value; select.append(option); }
  select.addEventListener('change', () => { onChange(select.value || undefined); recordingComparison = undefined; renderCurrentData(); }); field.append(select); return field;
}

function renderCompare(): HTMLElement {
  const section = element('section'); section.append(element('p', 'summary', '時間・イベント種別・Storageキー・API endpointをbest effortで対応付けます。表示は因果関係を証明しません。'));
  if (recordings.length < 2) { section.append(element('div', 'empty', 'Compareには2つの保存済みRecordingが必要です。')); return section; }
  const controls = element('div', 'focused-export-fields'); controls.append(recordingSelect(normalRecordingId, 'Normal recording', (id) => { normalRecordingId = id; }), recordingSelect(brokenRecordingId, 'Broken recording', (id) => { brokenRecordingId = id; }));
  const compare = element('button', 'action-button', 'Compare Recordings'); compare.type = 'button'; compare.addEventListener('click', () => { const normal = recordings.find((item) => item.id === normalRecordingId); const broken = recordings.find((item) => item.id === brokenRecordingId); if (normal && broken && normal !== broken) recordingComparison = compareRecordings(normal, broken); renderCurrentData(); }); controls.append(compare); section.append(controls);
  if (!recordingComparison) return section;
...
function timelineDetails(event: TimelineEvent): string {
  ...
  if (event.kind === 'frame-added' || event.kind === 'frame-navigated' || event.kind === 'frame-removed') {
    ...
    if (event.fromUrl) parts.push(`from: ${event.fromUrl}`);
    if (event.frame.parentFrameId !== undefined) parts.push(`parent: ${String(event.frame.parentFrameId)}`);
    return parts.join('\n');
  }
...
function timelineIcon(event: TimelineEvent): string {
  ...
  if (event.kind === 'frame-added') return '⊕';
  if (event.kind === 'frame-navigated') return '⇉';
  if (event.kind === 'frame-removed') return '⊖';
  return isFailureTimelineEvent(event) ? '✕' : '!';
}

function timelineTypeLabel(event: TimelineEvent): string {
  if (event.kind === 'user-action') return event.actionType.toUpperCase();
  if (event.kind === 'route-change') return 'ROUTE';
  if (event.kind === 'storage') return 'STORAGE';
  if (event.kind === 'network-request') return 'REQUEST';
  if (event.kind === 'network-response') return 'RESPONSE';
  if (event.kind === 'frame-added' || event.kind === 'frame-navigated' || event.kind === 'frame-removed') return 'FRAME';
  return 'ERROR';
}

/** Short label for a frame, used in UI badges and AI export. */
function frameLabel(event: TimelineEvent): string {
...
function renderDebugTimeline(): HTMLElement {
  ...
  const frameOptions = new Map<string, string>(); // key → display label
  ...
  if (frameOptions.size > 2) {
    ...
    const select = element('select', 'interval-select') as HTMLSelectElement;
  ...
  const events = allTimeline
    ...
    .filter((event) => {
      if (timelineFrameFilter === 'all') return true;
      if (timelineFrameFilter === 'main') return !event.frame || event.frame.isMainFrame;
  ...
  for (const event of events) {
    ...
    if (event.id === timelineFocusEventId) focusedRow = row;
...
function renderSnapshots(): HTMLElement {
  ...
  for (const [label, snapshot] of [['Raw Snapshot 1', beforeSnapshot], ['Raw Snapshot 2', afterSnapshot]] as const) {
    if (!snapshot) continue;
    const raw = element('details') as HTMLDetailsElement;
...
function renderFocusedExportControls(timeline: TimelineEvent[]): HTMLElement {
  ...
  if (focusedEventId && !currentFocusedEvent(timeline)) focusedEventId = undefined;
  ...
  const eventSelect = element('select', 'interval-select') as HTMLSelectElement;
  const fullOption = element('option') as HTMLOptionElement;
  fullOption.value = '';
  ...
  const beforeInput = element('input', 'compact-input') as HTMLInputElement;
  ...
  const afterInput = element('input', 'compact-input') as HTMLInputElement;
...
function renderCurrentData(): void {
  ...
  if (state.selected === 'storage') {
    setBody(renderInspectStorage(state.loadedData as InspectStorage));
    return;
  }
  ...
  if (state.selected === 'framework-state') {
    setBody(renderFrameworkStates(state.loadedData as FrameworkStates));
    return;
  }
}
...
async function refreshPanel(options: { background?: boolean } = {}): Promise<void> {
  ...
  if (target === 'storage') {
    ...
    state.loadedData = { local: local.data, session: session.data } satisfies InspectStorage;
    finish();
    return;
  }
  ...
  if (target === 'framework-state') {
    ...
    state.loadedData = { pinia: pinia.data, tanstackQuery: tanstackQuery.data } satisfies FrameworkStates;
    finish();
  }
}

...
function eventType(event: TimelineEvent): BridgeTimelineType { if (event.kind === 'storage') return 'storage-change'; if (event.kind.startsWith('network-')) return 'network'; if (event.kind === 'user-action' || event.kind === 'route-change') return event.kind; if (event.kind === 'frame-added' || event.kind === 'frame-navigated' || event.kind === 'frame-removed') return 'frame-lifecycle'; return 'error'; }
...
export async function handleBridgeRequest(request: BridgeRequest, source: BridgeStateSource): Promise<BridgeResponse> {
  ...
  if (request.method === 'getTimeline') {
    const types = request.params?.eventTypes; if (types && (!Array.isArray(types) || types.some((type) => !['user-action', 'route-change', 'storage-change', 'network', 'error', 'frame-lifecycle'].includes(type)))) return failure(request.requestId, 'INVALID_REQUEST', 'eventTypes contains an unsupported value.');

...
function time(value) { const parsed = Date.parse(value); return Number.isFinite(parsed) ? parsed : Number.POSITIVE_INFINITY; }
function endpoint(entry) { try {
    const url = new URL(entry.url);
    return `${entry.method} ${url.origin}${url.pathname}`;
}
catch {
    return `${entry.method} ${entry.url.split('?')[0]}`;
} }
function query(url) { try {
    return new URL(url).search;
}
catch {
    return url.includes('?') ? `?${url.split('?').slice(1).join('?')}` : '';
} }
function valueDiff(path, before, after) {
    if (JSON.stringify(before) === JSON.stringify(after))
        return [];
    if (before && after && typeof before === 'object' && typeof after === 'object' && !Array.isArray(before) && !Array.isArray(after)) {
        const keys = new Set([...Object.keys(before), ...Object.keys(after)]);
        return [...keys].flatMap((key) => valueDiff(`${path}.${key}`, before[key], after[key]));
    }
    return [{ path, before, after, kind: before === undefined ? 'added' : after === undefined ? 'removed' : 'changed' }];
}
function headers(entries) { return Object.fromEntries(entries.map((header) => [header.name.toLowerCase(), header.value])); }
function networkDiff(normal, broken) {
    return [
...
function parseJson(value) { if (!value)
    return value; try {
...
function storageKey(change) { return `${change.storageArea}.${change.key ?? 'clear'}`; }
export function findFirstDivergence(normal, broken) {
    ...
    for (const key of new Set([...normalStorage.keys(), ...brokenStorage.keys()])) {
        const left = normalStorage.get(key);
        const right = brokenStorage.get(key);
        if (left?.newValue !== right?.newValue)
            candidates.push({ timestamp: right?.timestamp ?? left?.timestamp ?? broken.createdAt, category: 'storage', key, normal: left?.newValue ?? null, broken: right?.newValue ?? null });
    }
    ...
    for (const kind of new Set([...normal.timeline.map((event) => event.kind), ...broken.timeline.map((event) => event.kind)])) {
        const leftEvents = normal.timeline.filter((event) => event.kind === kind);
        const rightEvents = broken.timeline.filter((event) => event.kind === kind);
        for (let index = 0; index < Math.max(leftEvents.length, rightEvents.length); index += 1) {
            const left = leftEvents[index];
            const right = rightEvents[index];
            if (!left || !right || left.summary !== right.summary)
                candidates.push({ timestamp: right?.timestamp ?? left?.timestamp ?? broken.createdAt, category: 'event', key: `${kind}[${index}]`, normal: left?.summary ?? null, broken: right?.summary ?? null });
        }
    }
    return candidates.sort((left, right) => time(left.timestamp) - time(right.timestamp))[0];
}
export function compareNetwork(normal, broken) {
    ...
    for (const entry of broken) {
        ...
        const changes = match ? networkDiff(match, entry) : [{ path: 'request', before: undefined, after: `${entry.method} ${entry.url}`, kind: 'added' }];
        if (changes.length)
            differences.push({ key, normal: match, broken: entry, differences: changes });
    }
    for (const [key, remaining] of available)
        for (const entry of remaining)
            differences.push({ key, normal: entry, differences: [{ path: 'request', before: `${entry.method} ${entry.url}`, after: undefined, kind: 'removed' }] });
    return differences;
}
export function buildRelatedEventChains(timeline) {
    const events = timeline.slice().sort((a, b) => time(a.timestamp) - time(b.timestamp));
    const chains = [];
    let current = [];
    for (const event of events) {
        const prior = current.at(-1);
        if (prior && time(event.timestamp) - time(prior.timestamp) > CHAIN_GAP_MS) {
            if (current.length > 1)
                chains.push({ id: chains.length + 1, events: current });
            current = [];
        }
        current.push(event);
    }
...
export function findSuspiciousEvents(recording, divergence) {
    const events = recording.timeline.slice().sort((a, b) => time(a.timestamp) - time(b.timestamp));
    const output = [];
    const add = (event, reason) => { if (event && !output.some((item) => item.event.id === event.id))
        output.push({ reason, event, previous: events[events.indexOf(event) - 1] }); };

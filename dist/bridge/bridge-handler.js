...
function limit(value) { const parsed = Number(value); return Number.isFinite(parsed) ? Math.min(AI_BRIDGE_MAX_LIMIT, Math.max(1, Math.floor(parsed))) : AI_BRIDGE_DEFAULT_LIMIT; }
function failure(requestId, code, message) { return { source: 'web-state-inspector-extension', type: 'response', requestId, success: false, error: { code, message } }; }
...
export async function handleBridgeRequest(request, source) {
    ...
    if (request.method === 'getNetworkErrors')
